...
`.zshrc` 用 `ys` 主题，prompt 带 docker 状态指示。Oh My Zsh 主题参考 (theme): `ys`。备选: powerlevel10k。
...
## 7. 龙芯 2K300/301 智能车平台**工具链**: `/opt/loongson-gnu-toolchain-8.3-x86_64-loongarch64-linux-gnu-rc1.6/`**SDK 源码结构** (龙邱科技 Loongson_2K300_301_Library):
...
| `tools/` | 交叉编译工具链 + 依赖库(ffmpeg/opencv/ncnn) |**久久派 25 集教程** (UP主: 龙邱科技):

| # | 内容 | # | 内容 |
|---|------|---|------|
| 1 | 常用 Linux 指令 | 2 | 程序编译流程 |
| 3 | GPIO 输出 | 4 | GPIO 输入 |
| 5 | PWM 功能 | 6 | GTIM 复用为 PWM |
| 7 | 编码器的使用 | 8 | 摄像头的使用 |
| 9 | ADC 功能 | 10 | SPI 屏显示 |
| 11 | 时间戳获取 | 12 | 各种延时函数 |
| 13 | OpenCV 交叉编译 | 14 | 串口功能 |
| 15 | 软件 I2C 驱动陀螺仪 | 16 | 陀螺仪驱动 |
| 17 | 开机自启 WIFI+SSH | 18 | 程序开机自启动 |
| 19 | TCP 网络通信 | 20 | UDP 网络通信 |
| 21 | 类定时器操作 | 22 | TFT 屏幕驱动 |
| 附 | YOLO 模型训练与龙芯部署推理 |

合集: https://space.bilibili.com/95313236/channel/collectiondetail?sid=4763568
...
**Debian 迁移注意**:- 工具链预编译于 x86_64，glibc 兼容性应无问题- ffmpeg/opencv/ncnn 预编译包基于 Ubuntu，Debian 下可能需重编- `build.sh` 中 `make -j` 参数在 Debian 上行为一致
